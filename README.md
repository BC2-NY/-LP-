...
```js
const news = [
  { date: '2026.07.10', tag: '重要', tagBorder: 'var(--accent-strong)', tagColor: 'var(--accent)', text: 'お知らせの本文' },
  // tag が「重要」以外なら tagBorder: 'var(--tag-border)', tagColor: 'var(--text-2)' を使う
];
```
...
配色はすべて `index.html` の `<style>` 冒頭にある **CSS変数** で管理しています。
`:root{}` がダークモード（既定）、`[data-theme=light]{}` がライトモードの配色です。色を変えたいときは変数の値だけを直します：

| 変数 | 用途 |
|---|---|
| `--bg` / `--bg-2` / `--bg-3` | ページ背景／カード背景／フッター背景 |
| `--text` / `--text-1〜3` | 文字色（基本／強→弱の3段階） |
| `--line` / `--line-strong` | 区切り線／枠線 |
| `--accent` / `--accent-strong` | アクセントの紫（見出し英字・ラベルなど） |
| `--sel-bg` / `--sel-fg` | テキスト選択時のハイライト色 |
| `--sb` | スクロールバーの色 |

- **ライト／ダーク切替**: ヘッダー右の ☀︎/☾ ボタン。初回は訪問者のOS設定に従い、手動で切り替えると `localStorage`（キー: `toitsu-theme`）に保存されて次回以降も維持されます。切替ロジックは `<head>` 内のインラインスクリプトにあります。
- **ヒーロー（トップの夜景）とモバイルメニューは常にダーク**のままにしてあるため、内部の色は変数化せず直書きです。ヒーロー内に文字を追加するときは `color:#fff` などを明示してください（bodyの文字色を継承させるとライトモードで沈みます）。
